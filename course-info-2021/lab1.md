...
**Running Individual(个人) Unit and System Tests**
...
<p>
You will need to fill in any piece of code that is not implemented(执行). It will be obvious where we think you should write
code. You may need to add private methods and/or helper classes. You may change APIs, but make sure
our [grading](#grading) tests still run and make sure to mention(提到), explain, and defend your decisions in your writeup.

<p>
In addition to the methods that you need to fill out for this lab, the class interfaces contain numerous(很多的) methods that
...
* Implement the classes to manage tuples(元组), namely Tuple, TupleDesc. We have already implemented Field, IntField,
...
Section 2 below walks you through these implementation steps and the unit tests corresponding(一致的) to each one in more
detail.
...
## 2. SimpleDB Architecture(架构) and Implementation Guide

SimpleDB consists of:

* Classes that represent fields, tuples, and tuple schemas(纲要);
...
* A catalog(目录) that stores(储存) information about available tables and their schemas.

SimpleDB does not include many things that you may think of as being a part of a "database."  In particular, SimpleDB
does not have:

* (In this lab), a SQL front end or parser（解析器） that allows you to type queries（疑问） directly into SimpleDB. Instead, queries are
  built up by chaining a set of operators（操作） together into a hand-built query plan (see [Section 2.7](#query_walkthrough)).
...
the collection of database file pages that are currently resident（居住） in memory), and the log file. You will not need to
...
policy is up to you; it is not necessary to do something sophisticated（复杂的）.
-->
...
include heap files (unsorted files of tuples) and B-trees; for this assignment（分配）, you will only implement a heap file
...
one `HeapFile` object for each table in the database. Each page in a `HeapFile` is arranged as a set of slots(槽), each of
which can hold one tuple (tuples for a given table in SimpleDB are all of the same size). In addition to(另外) these slots,
...
disk.
**测试**：HeapPageIdTest 、HeapPageReadTest、RecordIdTest
...
access to the file in order to read and write pages at arbitrary(任意) offsets. You should not call BufferPool methods when
...
Operators(操作符) are responsible for(负责) the actual execution of the query plan. They implement the operations of the relational algebra(代数). In SimpleDB, operators are iterator based; each operator implements the `DbIterator` interface.
...
Suppose(推断) you have a data file, "some_data_file.txt", with the following contents:
...
<p>
You can convert(转变) this into a binary file that SimpleDB can query as follows:
<p>
...
a `HeapFile` object representing(代表) the table stored in `some_data_file.dat`. Once we have created the table, we add it to
...
appropriate table (in the case of (在...情况下 )`SeqScan`) or child operator (in the case of e.g. Filter). The test program then
